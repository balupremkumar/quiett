...
# ---------------------------------------------------------------------------
# The reserved single key (numpad ".") was REMOVED 2026-08-23. Do not bring it
# back without reading the dossier at research/2026-08-23-dictation-insert-patterns
# in the vault.
#
# Two reasons it went, one a bug and one a design finding:
#
#  1. It could not be told apart from the app's own typing. inject.py types via
#     SendInput KEYEVENTF_UNICODE, which carries the UTF-16 code unit in wScan,
#     and ord("S") == 83 is the numpad "." scan code. The hook swallowed the S
#     out of the text it was inserting, so verification failed and the stash
#     stayed unconsumed, then re-fired place mode, which typed the same text
#     again. One press produced 14 inserts. Every candidate key had the same
#     hole: 78='N', 74='J', 55='7', 82='R', 69='E'.
#  2. No shipped dictation tool reserves an unmodified global key. All ten
#     surveyed auto-insert on release. Wispr Flow's hotkey validator refuses
#     single keys outright. The confirm gesture now lives where it always
#     belonged: Enter or Insert inside the preview panel, which already has
#     focus, so no global key is needed at all.
#
# If a global confirm is ever genuinely required, use RegisterHotKey with
# MOD_NOREPEAT on a chord. The OS de-duplicates auto-repeat, so a storm is
# impossible by construction. Do NOT reinstate a WH_KEYBOARD_LL hook for it.
#
# Place mode itself stays: the tray "Place last dictation" item, the recovery
# panel, the optional place_hotkey chord and the click-to-place capture above
# are all untouched.
# ---------------------------------------------------------------------------
